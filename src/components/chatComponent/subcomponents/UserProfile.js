...
export default function ProfileModal({
  ...
  // console.log("loggedUser", getSenderFull(user, chat.users)._id)
  //for avatar
  const StyledBadge = styled(Badge)(({ theme }) => ({
    "& .MuiBadge-badge": {
      backgroundColor: `${
        onlineUsers.some((u) => u._id === getSenderFull(user, chat.users)._id)
          ? "green"
          : "gray"
      }`,
      // color: "#44b700",
    ...
    <div>
      <Button onClick={handleClickOpen}>
        {!chat.isGroupChat ? (
          <StyledBadge
            ...
            variant="dot"
          >
            <Avatar alt="" src={`${getSenderFull(user, chat.users).image}`} />
          </StyledBadge>
        ) : (
          <img
      ...
      >
        <DialogTitle>
          ...
          {!chat.isGroupChat ? (
            <img
              ...
              src={`${getSenderFull(user, chat.users).image}`}
              alt=""
            />
          ) : (
            <img
        ...
        <DialogContent className="w-96">
          ...
          {!chat.isGroupChat && (
            <DialogContentText>
              {getSenderFull(user, chat.users).email}
            </DialogContentText>
          )}
        </DialogContent>
        <DialogActions sx={{ display: "flex", alignItems: "center" }}>
          <Button autoFocus onClick={handleClose} sx={{ fontSize: "12px" }}>
            Cancel
          </Button>
          {!chat.isGroupChat && (
            <Link
              className="shadow-md px-1 text-gray-500 "
              to={`/profile/blogger/${getSenderFull(user, chat.users)._id}`}
              onClick={handleClose}
            >
              Go to profile
            </Link>
